...
const Dashboard = () => {
  ...
  const formatIndianCurrency = (value) => {
    if (value >= 1e12) {
      return `${formatLargeNumber(
        (Math.round(value / 1e11) / 10).toFixed(2)
      )} Trillion`;
    } else if (value >= 1e9) {
      return `${formatLargeNumber(
        (Math.round(value / 1e8) / 10).toFixed(2)
      )} Billion`;
    } else if (value >= 1e6) {
      return `${formatLargeNumber(
        (Math.round(value / 1e5) / 10).toFixed(2)
      )} Million`;
    } else if (value >= 1e3) {
  ...
  const sidetabel = (selectedWard) => {
    ...
    if (selectedWard == null) {
      ...
      const Garbagepersons = fetchedData.filter(
        (entry) => entry.Garbage_Tax !== 0
      );
      const propertyrsons = fetchedData.filter(
        (entry) => entry.Property_Tax !== 0
      );

      document.getElementById("samarth").innerHTML = `Overall`;
      document.getElementById(
        "FirstBlock"
      ).innerHTML = `${waterpersons.length}/${totalpersons}`;
      document.getElementById(
        "SecondBlock"
      ).innerHTML = `${propertyrsons.length}/${totalpersons}`;
      document.getElementById(
        "ThirdBlock"
      ).innerHTML = `${Garbagepersons.length}/${totalpersons}`;
    }
    ...
    if (clicked !== null) {
      ...
      //   }
      // ]
      const TotalPersons = dataArray.filter(
        (entry) => entry.ward === `${selectedWard}`
      );

      const PaidWater = TotalPersons.filter((entry) => entry.Water_Tax !== 0);
      const PaidGarbage = TotalPersons.filter(
        (entry) => entry.Garbage_Tax !== 0
      );
      const PaidProperty = TotalPersons.filter(
        (entry) => entry.Property_Tax !== 0
      );

      document.getElementById(
        "FirstBlock"
      ).innerHTML = `${PaidWater.length}/${TotalPersons.length}`;
      document.getElementById(
        "SecondBlock"
      ).innerHTML = `${PaidGarbage.length}/${TotalPersons.length}`;
      document.getElementById(
        "ThirdBlock"
      ).innerHTML = `${PaidProperty.length}/${TotalPersons.length}`;
      document.getElementById(
        "samarth"
      ).innerHTML = `Ward Number : ${selectedWard}`;
    }
  };
  ...
  const fourbuttons = (selectedWard) => {
    ...
    if (selectedWard && fetchedData) {
      const wardData = fetchedData.filter(
        (entry) => entry.ward === `${selectedWard}`
      );

      if (wardData.length > 0) {
        const WardWaterTax = wardData.reduce(
          (total, entry) => total + entry.Water_Tax,
          0
        );
        const WardGarbageTax = wardData.reduce(
          (total, entry) => total + entry.Garbage_Tax,
          0
        );
        const WardPropertyTax = wardData.reduce(
        ...
        document.getElementById(
          "totalRevenueValue"
        ).innerText = `₹ ${formatIndianCurrency(TotalWardTax)}`;
        document.getElementById(
          "waterTaxValue"
        ).innerText = `₹ ${formatIndianCurrency(WardWaterTax)}`;
        document.getElementById(
          "garbageTaxValue"
        ).innerText = `₹ ${formatIndianCurrency(WardGarbageTax)}`;
        document.getElementById(
          "propertyTaxValue"
        ).innerText = `₹ ${formatIndianCurrency(WardPropertyTax)}`;
      }
    } else if (fetchedData != null && selectedWard == null) {
      const totalWaterTax = fetchedData.reduce(
        (total, entry) => total + entry.Water_Tax,
        0
      );
      const totalGarbageTax = fetchedData.reduce(
        (total, entry) => total + entry.Garbage_Tax,
        0
      );
      const totalPropertyTax = fetchedData.reduce(
        (total, entry) => total + entry.Property_Tax,
        0
      );
      ...
      document.getElementById(
        "totalRevenueValue"
      ).innerText = `₹ ${formatIndianCurrency(totalRevenue)}`;
      document.getElementById(
        "waterTaxValue"
      ).innerText = `₹ ${formatIndianCurrency(totalWaterTax)}`;
      document.getElementById(
        "garbageTaxValue"
      ).innerText = `₹ ${formatIndianCurrency(totalGarbageTax)}`;
      document.getElementById(
        "propertyTaxValue"
      ).innerText = `₹ ${formatIndianCurrency(totalPropertyTax)}`;
    } else {
